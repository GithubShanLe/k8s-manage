<template>
    ...
    <div class="filter-container">
      <el-select v-model="queryParams.nameSpace" placeholder="命名空间" class="filter-item" filterable  @change="handleNamespaceChange">
        ...
        <el-input
          v-model="queryParams.roleBindingName"
          placeholder="roleBinding名称"
          class="filter-item"
          style="width: 200px;"
          clearable
      ...
      <el-card shadow="hover" class="mt-20 info-card">
        <div slot="header" class="sub-header">
          <i class="el-icon-cpu"></i> rolebinding列表
          <span class="sub-title-count">({{ rolebindings.length || 0 }})</span>
        </div>
        <el-table 
          :data="rolebindings" 
          border 
  ...
  import { listRoleBinding,getNamespaces } from '@/api/k8s'
  export default {
  data() {
  return {
    listLoading: false,
  ...
  toggleLabels(index) {
    // 使用 Vue.set 确保响应式更新
    if (!this.rolebindings[index].labelsExpanded) {
      this.$set(this.rolebindings[index], 'labelsExpanded', true);
    } else {
      this.$set(this.rolebindings[index], 'labelsExpanded', false);
    }
  },  
  ...
  async fetchData() {
    ...
    try {
      const { data } = await listRoleBinding({
        roleBindingName: this.queryParams.roleBindingName
      })
      if (data.errorCode) {
      ...
      // 初始化每个服务的标签展开状态
      this.rolebindings = (data.roleBindings || []).map(rolebinding => ({
        ...rolebinding,
