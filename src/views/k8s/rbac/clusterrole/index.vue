<template>
    ...
    <div class="filter-container">
        <el-input
          v-model="queryParams.clusterRoleName"
          placeholder="clusterRole名称"
          class="filter-item"
          style="width: 200px;"
          clearable
      ...
      <el-card shadow="hover" class="mt-20 info-card">
        <div slot="header" class="sub-header">
          <i class="el-icon-cpu"></i> role列表
          <span class="sub-title-count">({{ clusterroles.length || 0 }})</span>
        </div>
        <el-table 
          :data="clusterroles" 
          border 
          ...
          <el-table-column label="Rules" min-width="200">
            <template slot-scope="{ row }">
              <div v-if="row.rules && row.rules.length" class="rules-container">
                <el-popover
                  placement="right"
                  width="400"
                  trigger="click"
                >
                      ...
                      <div class="rule-content">
                        <div class="rule-field">
                          ...
                          <div class="rule-values">
                            <el-tag v-for="(verb, i) in rule.verbs" :key="i" size="mini" type="success" class="rule-tag">
                              {{ verb }}
                            </el-tag>
                          </div>
                        </div>
                        <div class="rule-field">
                          ...
                          <div class="rule-values">
                            <el-tag v-for="(group, i) in rule.apiGroup" :key="i" size="mini" type="info" class="rule-tag">
                              {{ group||"\"\"" }}
                            </el-tag>
                          </div>
                        </div>
                        <div class="rule-field">
                          ...
                          <div class="rule-values">
                            <el-tag v-for="(resource, i) in rule.resource" :key="i" size="mini" type="warning" class="rule-tag">
                              {{ resource }}
                            </el-tag>
                          </div>
                        </div>
                  ...
                  <el-button slot="reference" type="text">查看规则 ({{ row.rules.length }})</el-button>
                </el-popover>
              </div>
              <span v-else class="muted-text">无规则</span>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
  ...
  import { listClusterRole } from '@/api/k8s'
  export default {
  data() {
  return {
    listLoading: false,
  ...
  toggleLabels(index) {
    // 使用 Vue.set 确保响应式更新
    if (!this.roles[index].labelsExpanded) {
      this.$set(this.roles[index], 'labelsExpanded', true);
    } else {
      this.$set(this.roles[index], 'labelsExpanded', false);
    }
  },  
  async fetchData() {
    ...
    try {
      ...
      const { data } = await listClusterRole({
        clusterRoleName: this.queryParams.clusterRoleName
      })
      if (data.errorCode) {
      ...
      // 初始化每个服务的标签展开状态
      this.clusterroles = (data.clusterRoles || []).map(clusterrole => ({
        ...clusterrole,
